#!/usr/bin/env python3
"""Generate platform branding assets from the approved CodeRoamer master icon."""
...
def extract_white_mark(image: Image.Image) -> Image.Image:
    """Extract the white glyph from the blue master with a soft antialiased edge."""
    rgb = image.convert("RGB")
    alpha = Image.new("L", rgb.size)
    source_pixels = rgb.load()
    alpha_pixels = alpha.load()

    # Blue background pixels have a low red/green minimum, while the white mark
    # has all three channels near 255. Remapping the minimum channel preserves
    # the original antialiasing without retaining the blue backdrop.
    transparent_at = 28
    opaque_at = 232
